...
func TestReviewToDomain_WithAllFields(t *testing.T) {
	...
	model := &models.ReviewModel{
		...
		Ease:       2500,
		TimeMs:     5000,
		Type:       valueobjects.ReviewTypeManual.String(),
		CreatedAt:  now,
	}
	...
	assert.Equal(t, valueobjects.ReviewTypeManual, entity.GetType())
...
func TestReviewToModel_WithAllFields(t *testing.T) {
	...
	entity, err := review.NewBuilder().
		...
		WithEase(2500).
		WithTimeMs(5000).
		WithType(valueobjects.ReviewTypeManual).
		WithCreatedAt(now).
		Build()
	require.NoError(t, err)
	...
	assert.Equal(t, valueobjects.ReviewTypeManual.String(), model.ReviewType)
	assert.Equal(t, now, model.CreatedAt)
}

func TestReviewToDomain_AllReviewTypes(t *testing.T) {
	reviewTypes := []valueobjects.ReviewType{
		valueobjects.ReviewTypeManual,
		valueobjects.ReviewTypeScheduled,
