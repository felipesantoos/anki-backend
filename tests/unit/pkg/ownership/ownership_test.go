...
func TestValidateOwnership(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ownership.ValidateOwnership(tt.userID, tt.resourceUserID)
			if got != tt.want {
...
func TestEnsureOwnership(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			resourceUserID: 2,
			wantErr:        true,
			wantErrType:    ownership.ErrAccessDenied,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ownership.EnsureOwnership(tt.userID, tt.resourceUserID)
...
func TestValidateOwnershipSlice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ownership.ValidateOwnershipSlice(tt.userID, tt.resourceUserIDs)
			if (err != nil) != tt.wantErr {
...
func TestErrResourceNotFound(t *testing.T) {
	err := ownership.ErrResourceNotFound
